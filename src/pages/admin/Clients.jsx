  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <EmptyState icon={Users} title="No clients yet" description="Client accounts will appear here once they sign up." />
      ) : (
        <Card padding={false}>
            ...
            {users.map(u => (
              <div key={u.id} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-hub-text-muted truncate">{u.businessName} · {u.city}, {u.state}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-hub-text-muted">{formatDate(u.createdAt)}</p>
                  <Badge variant={u.purchases?.citationsPackageId ? 'success' : 'gray'} className="mt-1">
                    {u.purchases?.citationsPackageId ? 'Citations' : 'Free'}
                  </Badge>
                </div>
              </div>
            ))}
